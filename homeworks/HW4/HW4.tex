...
\newline
\newline
At the very basic case, an anonymous function is nothing more but a body of a function definition. Since it is not associated with any identifier (i.e. when it is unnamed), we may use it only once - when we explicitly make it a part of an expression:
\ttfamily
...
\rmfamily
\noindent - also, an anonymous function can be used in cases when we would otherwise have to define an inner function:
